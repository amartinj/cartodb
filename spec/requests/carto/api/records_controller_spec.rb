...
describe Carto::Api::RecordsController do
  def create_token(permission)
    params = { api_key: @user.api_key, table_id: @table.name, user_domain: @user.username, permission: permission }
  ...
  describe '#show Token tests' do
    ...
    it 'Create a new row of type number and insert float values' do
      payload = {
          name:   'My new imported table',
          schema: 'name varchar, age integer',
          api_key: @user.api_key   #This test won't work without the api_key since it's outside of records_controller
      }
      ...
      # this test uses its own table
      table = Carto::Helpers::TableLocator.new.get_by_id_or_name(table_name, @user)
      token = Carto::UserTableToken.new(table: table, write_access: true)
      token.generate_value!

      custom_params = params.merge(table_id: table_name, user_token: token.value)
